...
btnLogin.addEventListener('click', function (e) {
  ...
  if (currentAccount?.pin === Number(inputLoginPin.value)) {
    //? Display UI with a welcome message
...
//? Continue and Bread does not work with forEach loop
*/
...
const checkDogs = function (dogsJulia, dogsKate) {
  const dogsAge = dogsJulia.slice(1, -2).concat(dogsKate);
...
//? Reduce --> reduce boils ("recudes") all array elements down to one sing value (e.g. adding all elements together)
...
const calcAverageHumanAge = function (dogsAges) {
  ...
  const avgHumanAge = adultDogs.reduce(
    (acc, adultDog, i, arr) => acc + adultDog / arr.length,
    0
  );
  console.log(avgHumanAge);
};
...
const eurToUsd = 1.1;
//? PIPELINE
// const totoalDepositsUSD = movements
//   .filter(mov => mov > 0)
//   .map(mov => mov * eurToUsd)
//   .reduce((acc, mov) => acc + mov, 0);
// console.log(totoalDepositsUSD);
...
//? Numbers
console.log(movements);
//? Converts everything to strings and sorts
...
//! Summary for all 23 array methods
...
const portion = function (dogs) {
  dogs.forEach(function (dog) {
    if (dog.curFood > dog.recommendedFood * 0.9) dog.portion = 'Too Much';
    else if (dog.curFood < dog.recommendedFood * 0.9)
      dog.portion = 'Too Little';
    else if (dog.curFood === dog.recommendedFood * 0.9) dog.portion = 'Exactly';
    else dog.portion = 'Okay';
  });
};
portion(dogs);
console.log(dogs);
...
//2.
const sarahDogPortion = dogs.find(dog => dog.owners.includes('Sarah')).portion;
console.log(sarahDogPortion);
